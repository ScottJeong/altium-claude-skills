...
근거 — 실제 진행에서 가안이 **4번 바뀌었다.** 한 번에 맞은 적이 없다.
...
"굳이 그 자리에 둔 이유가 있나" 는 질문을 받고서야 이유가 없다는 걸 알았다.
...
실측 사례 — PHY 를 데이터시트 몸체 `4.00 × 4.00` 으로 잡았는데
실제 풋프린트는 `7.05 × 7.00` 이었다. **43% 과소.** 몸체를 랜드로 착각한 결과다.

실측 사례 — 라이브러리를 못 찾아 공칭으로 그렸고, 대가는:
...
실측 사례 — 패시브 88개를 4회 고쳐 겹침 0 까지 갔지만
판정은 **"큰 부품은 쓸 만한데 패시브는 수동이 낫다"** 였다.
「주인 IC 근처에 겹치지 않게만 놓아주면 나머지는 손으로 한다」 는 것이다.
