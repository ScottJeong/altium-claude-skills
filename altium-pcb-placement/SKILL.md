...
근거 — 이 절차를 만든 세션에서 가안이 **4번 바뀌었다.** 한 번에 맞은 적이 없다.

| 회차 | 바뀐 것 | 누가 |
|---|---|---|
| 1 | 초안 (커넥터를 하변에) | 나 |
| 2 | 큰 커넥터를 좌변 세로, 나머지 커넥터는 상변 한 곳으로 | **사용자** |
| 3 | 소켓 회전 지정 (주요 부품 기준으로) | **사용자** |
| 4 | 보드 120→90 축소, TP 뱅크 조이기 | **사용자** |

4회차의 "보드가 30mm 크다" 는 내가 **부품 하나를 필요 없이 대각선에 둬서** 생긴 것이었다.
사용자가 "굳이 그 자리에 둔 이유라도?" 라고 물어서야 이유가 없다는 걸 알았다.
...
실증 — 이 절차를 만든 세션에서 PHY 를 데이터시트 몸체 `4.00 × 4.00` 으로 잡았는데
실제 풋프린트는 `7.05 × 7.00` 이었다. **43% 과소.** 몸체를 랜드로 착각한 결과다.

실증 — 이 절차를 만든 세션에서 라이브러리를 못 찾아 공칭으로 그렸고, 대가는:

| 부품 | 공칭 | 실측 |
|---|---|---|
| PHY | 4 × 4 (다이 크기를 씀) | **7.05 × 7.00** |
| DC 잭 | 14 × 11 | **9.90 × 14.40** (폭·깊이가 뒤집힘) |
| DIP 스위치 | 10 × 6 | **12.34 × 10.90** |

크기보다 나쁜 건, 실측을 미루면 **방향까지 기억으로 추론하게 된다**는 것이다.
커넥터 5V 핀이 어느 쪽으로 가는지를 내가 만든 생성 스크립트 기억으로 단정했다가
재보니 **반대**였다. 30초짜리 측정이었다.
...
그만큼 보드가 크다. 실증: 부품 하나를 이유 없이 대각선에 둬서 X 가 30mm 컸다.
...
실증 — 이 절차를 만든 세션에서 패시브 88개를 4회 고쳐 겹침 0 까지 갔지만
사용자 판정은 *"큰 것들은 나쁘지 않은데 소자들은 잘 안 되네. 관련 IC 근처에
겹치지 않게만 놓아주면 내가 수동으로 하는 게 좋겠다"* 였다.
