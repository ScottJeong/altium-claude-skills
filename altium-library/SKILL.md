...
- 새 부품을 끝내면 `index.md` 에 한 줄 추가한다 (산출물·상태·사내 반영처·미해결)
...
- 본체 사각형 **색** (`color` 테두리 / `area_color` 채움). 기본값을 쓰면 사내 심볼과 눈에 띄게 다르다
...
- 사내 라이브러리가 그 부품군을 기능 그룹으로 관리하고 있을 때
...
흔한 사고 — 데이터시트 탑뷰의 1번핀이 **좌하단**인데 심볼은 1번핀부터 **좌변 위**에 놓는 것.
반시계 순서가 같아 전기적으로는 문제없지만 **도면 대비 90° 돌아가서**
"칩과 같은 위치라 찾기 쉽다" 는 목적을 절반만 달성한다.
...
실적 예 — 32핀·최장 이름 18자에서 2700 정사각(어림) → **2300 정사각(계산)**, 면적 27% 감소.
...
렌더한 이미지를 눈으로 재는 것도 하지 마라. 한 세션에서 세 번 틀렸다.
...
- 사내 공용 3D 자산 폴더가 있으면 거기부터 본다 (`.../3D/<카테고리>/`).
...
(우리 EP 2.60/2.70/2.75 → `EP2.65x2.65` 채택, `EP2.9x2.9` 는 공차 밖이라 탈락).
