...
**`<작업폴더>` 는 라이브러리 작업을 모아 두는 곳이고 프로젝트 폴더와 별개다.**
스킬 폴더 안에 만들지 마라 (`git pull` 때 섞인다).

정해진 게 없으면 **사용자에게 묻고**, 답이 없으면 프로젝트 옆에
`altium-lib-work/` 를 만들어 쓰고 그 경로를 알린다. 한 번 정하면 계속 거기 쌓는다.

산출물과 중간물을 한 폴더에 몰면 부품 두세 개만 늘어도 뭐가 뭔지 못 찾는다.
(커넥터 하나에 파일 29개가 평평하게 쌓이는 일이 실제로 생긴다.)
...
## 4-B. 구리는 패드여야 한다 — 자유 구리는 배선을 막는다

풋프린트의 구리층(Top/Bottom)에 놓인 **region · arc · track 은 넷이 없다.** 라우터는
넷 있는 트랙을 넷 없는 구리에 붙이지 못한다.

패드가 그 자유 구리 **안에 묻혀 있으면** 바깥에서 오는 배선이 패드에 닿기 전에 그
구리를 만나 거부된다.
...
패드에서 출발하면 자기 풋프린트 구리 위에서 시작하는 거라 그냥 나간다. 그래서 라우터
버그나 레이어 문제로 오해하기 쉽다. **먼저 풋프린트의 구리층 프리미티브를 세라.**

```
python scripts/audit_free_copper.py <PcbLib 파일 또는 폴더> [--only NAME ...]
```

패드가 자유 구리에 **덮였는지**까지 보고 `배선 불가` / `주의` 로 가른다.
`주의` 는 자유 구리는 있지만 패드가 노출돼 있어 배선은 되는 경우다.

실제 사례 — 솔더브릿지 풋프린트에 0.20mm 패드 2개가 있고 그 위를 0.47mm 짜리
region 2개 + arc 5개가 덮고 있었다. 그 풋프린트를 쓴 17개가 전부 배선 불가였다.

### 고치는 법 둘

**A. 구리를 패드로 만든다.** region 윤곽을 커스텀 패드 형상으로 옮기고 자유 구리를
지운다. 자유 구리 0 이 되어 가장 깨끗하다.
커스텀 패드 제약은 `references/altium-monkey-api.md` 를 볼 것 — **새로 만들 수 없고
기존 것의 형상만 바꿀 수 있다.**

**B. 패드를 자유 구리 바깥으로 뺀다.** 벤더 라이브러리가 흔히 쓰는 방식이다.
실제 예 — ST 의 솔더브릿지는 표준 사각 패드를 양 끝에 두고 장식 구리를 그 사이에만
둔다. 배선은 패드 바깥쪽 변으로 들어오므로 막히지 않는다.

B 는 구리에 넷이 없는 상태로 남는다. **GND pour 를 깔면 그 주위로 클리어런스가 생기고
DRC 에 미결선 구리로 남는다.** pour 가 닿는 자리면 A 를 쓴다.
